...
function Gtext() {
    ...
    G.MM_items_2 = [
        ...
        ['separator'],  // разделительная линия
        ['Выгрузки TAM из Redash для КЦ (любой отчёт) [обновлено 19.01.2023]', 'SCR_redash_TAM'],
        ['Вечерние базы КЦ [обновлено 22.01.2024]',                            'SCR_evening_СС']]

    // others
    G.SCR_redash_TAM_confirm = '1. Скопировать выгруженные ID в таблицы отчётов Redash;\n2. Проверить категории и указать в заголовке нужного столбца "Категория"; другие столбцы категорий будут удалены;\n3. Скопировать нужные столбцы в файл для отправки письма в КЦ;\n4. Разделить таблицу на отдельные по 5-6 тысяч строк.\n\nНажмите "Да", чтобы запустить скрипт, или "Нет", чтобы отменить выполнение.';
    G.ask_CRMmrkg_type = 'Возможные варианты:\n• White;\n• Grey;\n• White/Grey (тот, который с приоритетами 1-4).\n\nЕсли нажать "Отмена", скрипт завершит работу, не внеся никаких изменений.';
